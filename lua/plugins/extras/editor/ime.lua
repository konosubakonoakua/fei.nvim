...
if jit.os == "Windows" then
  ...
  local ime_switch = function (mode)
    if mode == "en" then
      return vim.fn.pyeval("win32api.SendMessage(win32gui.GetForegroundWindow(), WM_INPUTLANGCHANGEREQUEST, 0, 0x0409)")
    elseif mode == "zh" then
      return vim.fn.pyeval("win32api.SendMessage(win32gui.GetForegroundWindow(), WM_INPUTLANGCHANGEREQUEST, 0, 0x0804)")
    else
      return -1
    end
  end
  ...
  vim.api.nvim_create_autocmd({"VimEnter", "InsertLeave"}, {
    pattern = "*", callback = function(ev) ime_switch("en") end,
  })
